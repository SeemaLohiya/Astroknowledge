...
import { BookOpen, Flame, Heart, Info, LogOut, Mail, Menu, ShoppingBag, ShoppingCart, Sparkles, X } from "lucide-react";
...
const MAIN_NAV_LINKS = [
  ...
  { href: "/products", key: "products" as const, icon: ShoppingBag },
] as const;
...
export function Header() {
  ...
  const renderNavLink = (link: (typeof MAIN_NAV_LINKS)[number] | (typeof DIRECT_LINKS)[number], mobile = false) => {
    ...
    return (
      ...
      >
        <Icon className={cn("opacity-70", mobile ? "h-5 w-5" : "h-4 w-4", active && "text-gold")} />
        {t(link.key)}
        {!mobile && active && (
      ...
      >
        <div className="mx-auto flex max-w-screen-2xl items-center justify-between gap-2 px-3 py-3 sm:px-4">
          ...
          <nav className="hidden lg:flex items-center gap-0.5">
            {MAIN_NAV_LINKS.map((link) => renderNavLink(link))}
            {DIRECT_LINKS.map((link) => renderNavLink(link))}
          </nav>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 bg-cream/98 overflow-y-auto lg:hidden animate-fade-in">
          <div className="flex flex-col p-6 pt-20 pb-32">
            ...
            {MAIN_NAV_LINKS.map((link) => renderNavLink(link, true))}
            {DIRECT_LINKS.map((link) => renderNavLink(link, true))}
