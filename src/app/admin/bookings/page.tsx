...
export default function AdminBookingsPage() {
  ...
  const filteredSlots = useMemo(() => {
    ...
    else if (slotTab === "completed") list = list.filter((s) => s.status === "completed");
  ...
  const handleBulkAdd = async (e: React.FormEvent) => {
    ...
    if (!res.ok || !data) {
      toast.error(data?.error || "Bulk create failed");
      return;
    }
  ...
  return (
    <PageTransition>
      <FadeIn className="mb-6">
        ...
        {pendingCount > 0 && (
          <div className="mt-3 flex items-center gap-2 rounded-xl bg-gold/10 border border-gold/30 px-4 py-2 text-sm text-gold">
            ...
            <span>{pendingCount} slot booking(s) awaiting confirmation</span>
          </div>
        )}
      </FadeIn>
        ...
        <form onSubmit={handleBulkAdd} className="rounded-2xl glass-card p-6 h-fit mt-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={bulk.skipSunday} onChange={(e) => setBulk({ ...bulk, skipSunday: e.target.checked })} /> Skip Sundays
            </label>
          </div>
          <button type="submit" className="mt-4 rounded-full bg-gold px-4 py-2 text-sm font-bold text-white">
            Create bulk slots
          </button>
        </form>
      </div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {(["open", "pending", "booked", "completed"] as SlotTab[]).map((t) => (
          <button
            key={t}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gold">
                    {c.booking.duration}: {slot.duration}
                  </p>
                  {slot.userName && (
                    <>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {slot.status === "pending" && (
                  <>
                    <button
                      onClick={() => {
                        slotAction(slot.id, { action: "confirm" });
                        toast.success("Booking confirmed");
                      }}
                      className="flex items-center gap-1 rounded-full bg-green-600 px-3 py-1 text-xs text-white hover:bg-green-700"
                    >
                    ...
                    {slot.userPhone && (
                      <a
                        href={buildWhatsAppUrl(
                          bookingConfirmMessage(slot.userName || "", slot.serviceName || "Consultation", slot.date, slot.time),
                          slot.userPhone.replace(/\D/g, "")
                        )}
                        target="_blank"
                        rel="noopener"
                    ...
                    <button
                      onClick={() => {
                        slotAction(slot.id, { action: "reject" });
                        toast.success("Booking rejected");
                      }}
                      className="flex items-center gap-1 rounded-full bg-red-500/20 px-3 py-1 text-xs text-red-500 hover:bg-red-500/30"
                    >
                ...
                {(slot.status === "booked" || slot.status === "pending") && (
                  <button
                    onClick={() => void slotAction(slot.id, { status: "completed" })}
                    className="flex items-center gap-1 rounded-full bg-emerald-600 px-3 py-1 text-xs text-white hover:bg-emerald-700"
                    title="Mark completed so the user can rebook"
                  >
                    <CheckCircle2 className="h-3 w-3" /> Complete
                  </button>
                )}
                {slot.status === "booked" && (
                  <button onClick={() => slotAction(slot.id, { action: "reject" })} className="text-xs text-gold hover:underline">
                    {c.admin.release}
                  </button>
                )}
                {slot.status === "available" && (
                  <button onClick={() => slotAction(slot.id, { status: "blocked" })} className="text-xs text-text-muted hover:underline">
                    {c.admin.block}
                  </button>
                )}
                {slot.status === "blocked" && (
                  <button onClick={() => slotAction(slot.id, { status: "available" })} className="text-xs text-gold hover:underline">
                    {c.admin.release}
                  </button>
                )}
                <button onClick={() => handleDeleteSlot(slot.id)} className="p-1 text-red-400 hover:bg-red-400/10 rounded">
