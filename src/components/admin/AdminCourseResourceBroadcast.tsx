...
export function AdminCourseResourceBroadcast({ courses }: AdminCourseResourceBroadcastProps) {
  ...
  const broadcast = async () => {
    ...
    try {
      const res = await fetch("/api/admin/courses/resources", {
        ...
        body: JSON.stringify({ courseId, links }),
      });
      const data = await parseResponseJson<{ message?: string; error?: string; updatedCount?: number }>(res);
      if (!res.ok) throw new Error(data?.error || "Failed to send");
      toast.success(data?.message || "Links sent to all enrolled students");
      setRows([{ label: "", url: "" }]);
    } catch (e) {
  ...
  return (
    <div className="mt-8 rounded-2xl border border-gold/20 bg-gradient-to-br from-gold/5 to-orange/5 p-6">
      <h2 className="mb-1 flex items-center gap-2 font-semibold text-text-primary">
        <Link2 className="h-5 w-5 text-gold" />
        Send resources to all enrolled students
      </h2>
      <p className="mb-4 text-xs text-text-muted">
        Links are added to every user who has paid for the selected course. They appear under Resources on
        their Courses dashboard.
      </p>
      ...
      <div className="mb-4">
        ...
        <select
          ...
          className="w-full max-w-md rounded-xl border border-gold/20 bg-white px-3 py-2.5 text-sm"
        >
          {courses.map((c) => (
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        >
          ...
          {sending ? "Sending…" : "Send to all enrolled users"}
        </button>
      </div>
    </div>
  );
