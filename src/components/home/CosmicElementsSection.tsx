...
export function CosmicElementsSection() {
  ...
  return (
    <section
      className="navgraha-cosmos relative overflow-hidden py-24"
      style={planetStyle}
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-4">
        ...
        <RevealOnScroll className="mt-10">
          <div className="navgraha-zodiac-belt relative overflow-hidden rounded-2xl border border-gold/15 bg-white/50 px-4 py-5 backdrop-blur-sm">
            <p className="mb-3 text-center text-[10px] font-bold uppercase tracking-[0.2em] text-gold">12 Rashis · Zodiac Wheel</p>
            <div className="navgraha-zodiac-track flex flex-wrap justify-center gap-2">
              {ZODIAC_SIGNS.map((sign, i) => (
                <span
                  key={sign.sign}
                  className="navgraha-zodiac-pill group/z rounded-full border border-gold/25 bg-gradient-to-br from-orange/5 to-white px-3 py-1.5 text-sm text-text-body transition-all hover:-translate-y-1 hover:scale-110 hover:border-gold/50 hover:bg-gold/10 hover:text-gold hover:shadow-md"
                  style={{ animationDelay: `${i * 0.04}s` }}
                >
                  <span className="inline-block transition-transform duration-300 group-hover/z:rotate-12">{sign.symbol}</span>{" "}
                  {sign.sign}
                </span>
              ))}
            </div>
